...
viewof dataVersion = {
  if (!config || !selectedNowcastDate || !config.as_of_dates) {
    return Inputs.radio(["round-open", "latest"], { value: "round-open" });
  }
  ...
  return Inputs.radio(["round-open", "latest"], {
    value: "round-open",
    format: d => d === "round-open" ? roundOpenLabel : latestLabel
  });
}
```
...
```{ojs}
//| echo: false
viewof showIntervals = Inputs.toggle({
...
```{ojs}
//| echo: false
viewof intervalLevel = Inputs.select([50, 80, 95], {
  value: 80,
  format: d => `${d}%`,
...
```{ojs}
//| echo: false
// Prediction intervals require latest data (observations after nowcast date)
predictionIntervalsAvailable = dataVersion === "latest"

viewof intervalType = {
  const container = html`<div style="line-height: 1.4;"></div>`;

  const options = [
    { value: "confidence", label: "Confidence intervals", enabled: true },
    { value: "prediction", label: "Prediction intervals", enabled: predictionIntervalsAvailable }
  ];

  options.forEach((opt, i) => {
    const id = `interval-type-${opt.value}`;
    const disabled = !opt.enabled || !showIntervals;
    const label = html`<label style="display: block; margin-bottom: 0.1rem; ${disabled ? 'opacity: 0.4; cursor: not-allowed;' : ''}">
  ...
  container.oninput = () => {
    ...
    container.value = checked ? checked.value : "confidence";
  };

  return container;
}

// Force confidence intervals when prediction intervals not available
effectiveIntervalType = predictionIntervalsAvailable ? intervalType : "confidence"
```
...
viewof selectedModels = {
  ...
  const updateValue = () => {
    ...
    if (window._dashboardState) window._dashboardState.modelSelections = container.value;
...
// Use relative URLs for local development, GitHub raw URLs for production
baseUrl = window.location.hostname === "localhost" ? "" : "https://raw.githubusercontent.com/reichlab/variant-nowcast-hub-dashboard/dashboard-data"
...
targetData = {
  if (!selectedLocation || !selectedNowcastDate || !dataVersion) return null;
  if (typeof selectedLocation !== 'string' || typeof selectedNowcastDate !== 'string') return null;
  try {
    const url = `${baseUrl}/targets/${dataVersion}/${selectedLocation}_${selectedNowcastDate}.json`;
    console.log("Fetching target:", url);
    const response = await fetch(url);
    if (!response.ok) {
      console.log("Target fetch failed:", response.status);
      return null;
    }
    return await response.json();
  } catch (e) {
...
getQuantileCols = (level, type) => {
  ...
  const mapping = {
    ...
    80: { lower: `${prefix}0.1`, upper: `${prefix}0.9` },
    95: { lower: `${prefix}0.025`, upper: `${prefix}0.975` }
  };
  return mapping[level] || mapping[80];
}
  ...
  if (targetData && targetData.data) {
    selectedClades.forEach((clade, cladeIdx) => {
      ...
      traces.push({
        ...
        xaxis: xaxis,
        yaxis: yaxis,
        hovertemplate: `Observed<br>Date: %{x}<br>Proportion: %{y:.2f}<br>Count: %{customdata[0]}/%{customdata[1]}<extra></extra>`
      });
    });
  }
...
buildLayout = () => {
  ...
  // Add vertical line for nowcast date
  if (selectedNowcastDate) {
    layout.shapes = [{
      type: "line",
...
hasTargetData = {
  ...
  // Check if any clade has non-zero total counts
...
- **Shaded ribbons** show prediction intervals at the selected confidence level
- **Black dots** show observed data; dot size reflects the total number of sequences for that week
- **Vertical dashed line** indicates the nowcast/submission date
- **Latest data** shows the most recent observations; **Round-open data** shows what was available when the nowcast was made
