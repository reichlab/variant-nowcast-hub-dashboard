  ...
  if (targetDataByVersion && Object.keys(targetDataByVersion).length > 0) {
    ...
    for (const [version, targetData] of Object.entries(targetDataByVersion)) {
      ...
      selectedClades.forEach((clade, cladeIdx) => {
        ...
        // Scale marker size by total count (keep minimum 4px so zero-data points remain hoverable)
        ...
        // Per-point colors: transparent for zero-total points, normal style color otherwise
        const isZero = cladeTargetData.total.map(t => t === 0);
        const markerColors = isZero.map(z => z ? "rgba(0,0,0,0)" : style.color);
        const lineColors   = isZero.map(z => z ? "rgba(0,0,0,0)" : (style.line.color || style.color));
        ...
        traces.push({
          ...
          marker: {
            color: markerColors,
            line: { ...style.line, color: lineColors },
