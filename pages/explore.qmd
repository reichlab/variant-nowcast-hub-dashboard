...
viewof intervalLevel = {
  ...
  const initialValue = savedValue !== undefined ? savedValue : 80;

  const select = Inputs.select([50, 80, 95], {
    value: initialValue,
    format: d => `${d}%`,
...
viewof intervalType = {
  ...
  // Get saved value or default to confidence
  ...
  options.forEach((opt, i) => {
    ...
    // Use saved value if available and option is enabled, otherwise use first enabled
    const isChecked = !disabled && opt.value === initialValue;
  ...
  // If no radio is checked, check the first enabled one
...
baseUrl = window.location.hostname === "localhost" ? "/output" : "https://raw.githubusercontent.com/reichlab/variant-nowcast-hub-dashboard/dashboard-data"
